 ...
 * Esta función tiene como objetivo:
 ...
 Verificar si se está ejecutando en un navegador.
 Obtener un token de autorización desde localStorage (si está disponible).
 Si el token existe, se añade a las cabeceras bajo la clave Authorization.
 Devolver un conjunto de cabeceras que pueden usarse en una solicitud HTTP.

 La línea if (typeof window !== 'undefined' && !headers.Authorization) verifica si el código se está ejecutando en un entorno del navegador (ya que window es un objeto global que solo existe en el navegador). Esto es necesario porque el acceso a localStorage solo está disponible en navegadores y no en entornos de servidor, como Node.js.
 También verifica si no hay ya un encabezado Authorization definido en el objeto headers
